...
async function embedDocument(doc: Document): Promise<PineconeRecord> {
  try {
    ...
    // Check if embedding is undefined or not an array
    if (embedding === undefined || !Array.isArray(embedding)) {
      throw new Error(
        'Failed to generate embedding: Embedding is undefined or not an array'
      )
    }
