...
export default function Messages({
  ...
  messages: Message[]
  withContext: boolean
  context?: { context: PineconeRecord[] }[]
}) {
                      ...
                      return (
                        <div key={index}>
                          <button
                            onMouseEnter={(
                              event: React.MouseEvent<HTMLButtonElement>
                            ) => handleClick(event, message.id, chunk.id)}
                            onMouseLeave={() =>
                              handleClose(message.id, chunk.id)
                            }
                            className="ml-2"
                          >
                            [
                            <span className="px-0.5 text-[#1B17F5] underline">
                              {index + 1}
                            </span>
                            ]
                          </button>
