...
const Chat: React.FC<ChatProps> = forwardRef<ChatInterface, ChatProps>(
  ({ withContext, setContext, context }, ref) => {
    ...
    return (
      <div
        className="flex-col w-50 overflow-auto h-full"
        style={{ borderLeft: '1px solid #738FAB1F' }}
      >
        <div
          className={`${messages.length == 0 ? 'flex flex-col justify-center items-center h-full' : 'overflow-auto'}`}
        >
          {context ? (
            <Messages
              messages={messages}
