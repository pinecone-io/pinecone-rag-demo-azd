...
const styles: Record<string, React.CSSProperties> = {
  contextWrapper: {
    display: 'flex',
    padding:
      'var(--spacer-huge, 64px) var(--spacer-m, 32px) var(--spacer-m, 32px) var(--spacer-m, 32px)',
...
export const Sidebar: React.FC = () => {
  ...
  const [clearIndexComplete, setClearIndexCompleteMessageVisible] =
  ...
  const handleSplittingMethodChange = (
    event: SelectChangeEvent<typeof splittingMethod>
  ) => {
    const {
      target: { value }
    } = event
  ...
  const menuItems = entries.map((entry, key) => (
    <MenuItem key={key} value={entry.url}>
      <div className="flex-col" data-testid={entry.url}>
        <div>{entry.title}</div>
        <div style={{ ...styles.entryUrl, whiteSpace: 'nowrap' as 'nowrap' }}>
          {entry.url}
        </div>
      </div>
    </MenuItem>
    ...
    >
      ...
      >
        ...
        <div style={{ marginTop: 24, marginBottom: 24 }}>
          This RAG chatbot uses Pinecone and OpenAI to demonstrate a URL crawl,
          data chunking and embedding, and semantic questioning.
        </div>
      </div>
      <div
        className="flex flex-column w-full"
        style={{ ...styles.textHeaderWrapper, flexDirection: 'column' }}
      >
          ...
          >
            ...
            >
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ fontSize: 14, color: 'grey' }}>
                  Markdown chunking leverages the structure of the document
                  itself, creating chunks that correlate to the markdown
                  semantics of the content. The crawler converts the URL content
                  to markdown and then applies the markdown chunking methods on
                  the content.
                </div>
              </div>
            </MenuItem>
            ...
            >
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ fontSize: 14, color: 'grey' }}>
                  With recursive chunking, the text will be divided into smaller
                  parts in each recursion step, based on the chunk size you
                  define. The overlap will ensure that the chunks include
                  content found in adjacent chunks so that no content is lost.{' '}
                </div>
              </div>
            </MenuItem>
          </Select>
      ...
      <div className="flex flex-wrap w-full mt-5 border-b border-[#738FAB1F]">
        ...
        <div
          className="text-[#1B17F5] ml-auto cursor-pointer text-xs"
          onClick={handleClearIndexClick}
        ...
        <div className="flex">
          {cards && cards.length > 0 ? (
            <div className="mt-2 flex flex-row">
              <div className="font-semibold mb-4 whitespace-nowrap">
